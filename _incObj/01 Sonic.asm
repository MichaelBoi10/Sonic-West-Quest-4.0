...
Sonic_Main:	; Routine 0
		cmpi.w	#id_GHZ_act1,($FFFFFE10).w ; set level to	GHZ (00)
		bne.s .fuckoff
...
Sonic_Balance:
		...
		bne.s	Sonic_BalanceLeft				; if not, check for the left edge
...
Sonic_BalanceLeft:
		btst	#0,obStatus(a0)		; is Sonic facing left?	;Mercury Constants
		beq.s	Sonic_BalanceBackward		; if not, balance backward
		move.b	#id_Balance,obAnim(a0)	; use forward balancing animation
...
Sonic_BalanceBackward:
		move.b	#id_Balance2,obAnim(a0)	; use backward balancing animation
...
.loadframe:
		...
		bhs.s	.end_FF			; MJ: if so, branch to flag routines				; if animation is complete, branch
